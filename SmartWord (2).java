...
public class SmartWord {
	...
	public String[] guess (char letter, int letterPosition, int wordPosition) {
		...
		double upperLimit = averageSentenceLength + (averageSentenceLength / 3.0);
		double lowerLimit = averageSentenceLength - (averageSentenceLength / 3.0);
		if (wordPosition <= lowerLimit) {
			...
			//relativeWordPos = wordPosition / averageSentenceLength;
			relativeWordPos = wordPosition / (upperLimit-lowerLimit);
		}
		//(averageSentenceLength + (averageSentenceLength / 3.45))
		if (letterPosition == 0) {
	...
	public void feedback (boolean isCorrectGuess, String correctWord) {
		if (isCorrectGuess) {
			...
			if (index == -1) {
				...
				//System.out.printf("index of y is %d; word there is %s%n", letterIndices[24], dictionary.get(letterIndices[24]).getWord());
			} else {
	...
	public static int findInsertPos (String word) {
		...
		//System.out.printf("low is %d; dictionary.get(low-1) is %s; word is %s%n",low, dictionary.get(low - 1).getWord(), word);
		return low - 1;
	}
